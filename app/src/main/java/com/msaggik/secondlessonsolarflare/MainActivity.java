...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] volumeData = {14, 1250, 883}; // количество информации в час с Марса, Юпитера, Сатурна (в эксабайтах)

    private int coreFrequency = 3; // скорость компьютера в эксабайт/секунду
...
//        }
        ...
        int[] volume = new int[volumeData.length]; // создание пустого массива для времени обработки компьютером данных от спутника (в секундах)
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i] / 60 * solarTime[i]); // определение времени обработки компьютером данных от одного спутника
        }
